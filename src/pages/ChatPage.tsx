...
export default function ChatPage() {
  ...
  const renderPanel = (panel: PanelType) => {
    ...
    if (panel === 'itinerary' && itinerary) {
      return <ItineraryPanel className="h-full" data={itinerary} onOpenDetails={handleOpenPOI} onItineraryUpdate={setItinerary} />
    }
  ...
  const renderDesktop = () => {
    ...
    const rightContent = selectedPoi ? (
      loadingPoi ? (
        ...
        <POIPanel poi={selectedPoi} size="half" onClose={() => setSelectedPoi(null)} showToggleFullWidth={false} showAddToTrip={false} />
      )
    ) : (
      ...
      <div className="hidden h-full flex-col lg:flex">
        <div className="z-10 flex h-12 flex-shrink-0 items-center border-b bg-white px-4">
          <div className="flex h-full w-1/2 items-center justify-between border-r pr-4">
            ...
            <Tabs value={leftPanel} onValueChange={(value) => handleTabChange('left', value as PanelType)} hidden={!itinerary}>
          ...
          <div className="flex h-full w-1/2 items-center justify-end" hidden={!itinerary}>
            <Tabs value={rightPanel} onValueChange={(value) => handleTabChange('right', value as PanelType)} hidden={!!selectedPoi}>
              <TabsList className="h-8">
  ...
  const renderMobile = () => {
    ...
    if (mobileTab === 'chat') {
      ...
      content = <ItineraryPanel className="h-full" data={itinerary} onOpenDetails={handleOpenPOI} onItineraryUpdate={setItinerary} />
